     ...
     top_concerns_list = [
       [Survey::Question.find_by_sql(["select sq.text FROM  survey_questions sq  join survey_answers sa on sa.question_id = sq.id
            join survey_surveys s on sq.survey_id = s.id
             ...
             where sa.option_id =1 and sat.participant_id = ? and s.id= ?", self.participant_id, self.survey.id]).map {|x| x.text << " (strongly disagree)"}],
       [Survey::Question.find_by_sql(["select sq.text FROM  survey_questions sq  join survey_answers sa on sa.question_id = sq.id
            join survey_surveys s on sq.survey_id = s.id
             join survey_attempts sat on sa.attempt_id = sat.id
             where sa.option_id =2 and sat.participant_id = ? and s.id= ?", self.participant_id, self.survey.id]).map {|x| x.text << " (disagree)"}],
       [Survey::Question.find_by_sql(["select sq.text FROM  survey_questions sq  join survey_answers sa on sa.question_id = sq.id
            join survey_surveys s on sq.survey_id = s.id
             join survey_attempts sat on sa.attempt_id = sat.id
